	...
	*/
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('BiblioTec_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('BiblioTec_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['BiblioTec_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['BiblioTec_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> BiblioTec</a>
			</div>
